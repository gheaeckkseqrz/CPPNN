...
int main(int ac, char ** av)
{
  ...
  if (ac < 3)
    {
      std::cerr << "Usage : " << av[0] << " CONTENT_IMAGE STYLE_IMAGE" << std::endl;
      return 1;
    }
  ...
  std::shared_ptr<NN::Tensor> content = tensorFromImage(av[1], 512);
  ...
  for (int i(0) ; i < 5 ; i++)
    {
      ...
      result->clamp(0, 1);
      result->mul(255);
      saveTensorAsImage(result, "result" + std::to_string(i) + ".png");
      result = resizeTensor(result, 512, 512);
      result->clamp(0, 255);
      result = blendImage(result, content, .8f);
      saveTensorAsImage(result, "result_content" + std::to_string(i) + ".png");
    }
  return 0;
}
